...
notifications:Notify("Fate Is Loading...");
   local cam = workspace.CurrentCamera
    ...
    Image.Image = "rbxassetid://17178655518"
...
notifications:Notify("Fate Loaded");
...
getgenv().fateConnections = {}
do
    function fateConnections:DisconnectAll()
        for _, connection in next, fateConnections do
...
function event:new(eventName): listener
    ...
    function listener:bindConnection(connection, identifier, callback): bind_connection
        ...
        fateConnections[identifier] = connection
        table.insert(fateConnections, connection)
        return connection
    end
    function listener:unbindConnection(identifier): unbind_connection
        if fateConnections[identifier] then
            local function unBind(connection)
                return connection:Disconnect()
            end
            unBind(fateConnections[identifier])
        end
    end
    return listener
end
...
function config:getFolder()
    ...
    for i, v in next, listfiles("fate/configs") do
        local filename = v
        local result = string.gsub(filename,"fate/configs.*\\","")
...
-- Function to handle commands entered in the chat
local function onChat(command)
    if command == "$rev" then
        teleportToCoordinates(-642, 22, -124)
    elseif command == "$tac" then
        teleportToCoordinates(482, 48, -602)
    elseif command == "$db" then
        teleportToCoordinates(-1042, 22, -261)
    elseif command == "$downhill" then
        teleportToCoordinates(-565, 8, -737)
    elseif command == "$bank" then
        teleportToCoordinates(-446, 39, -286)
    end
end

-- Hooking into the chat system
...
aimassistCombat:Label({ Name = "OH YEAAA FATE OT"})
...
miscVisuals:Toggle({Name = "Field Of View", Default = false, Pointer = "fieldofviewActive", callback = function(bool)
    if bool then
        ...
        if fateConnections["xzbnciyuasbyid"] then fieldofviewEvent:unbindConnection("xzbnciyuasbyid") end
...
worldVisuals:Toggle({ Name = "Time of Day", Pointer = ")timeofday", callback = function(state)
    if state then
        ...
        if fateConnections["timeofdayConn"] then timeofdayEvent:unbindConnection("timeofdayConn") end
...
local pageTextures = library:Page({ Name = "Textures & Sky" })
local cfgTextures = pageTextures:Section({ Name = "Make Textures" })

local selectedMaterial = "Brick" -- Default material
local selectedColor = Color3.fromRGB(255, 255, 255) -- Default color
...
cfgTextures:Dropdown({
    Name = "Material",
    Options = {"Asphalt", "Basalt", "Brick", "Cobblestone", "Concrete", "Granite", "Grass", "Ice", "Marble", "Mud", "Pavement", "Rock", "Snow", "Wood", "WoodPlanks"},
    Default = "Brick",
    Pointer = " ",
...
local silentaimCombat = pageTextures:InDevSection({ Name = "Textures", Side = "Right" }) -- Displaying within the "Textures" page

-- Adding a new button called "Minecraft" to the "Silent Aim" section
silentaimCombat:Button({
    Name = "Minecraft",
    Callback = function()
        -- Add your Minecraft button logic here
        loadstring(game:HttpGet("https://raw.githubusercontent.com/Leavinbodys/Fate/main/Minecraft"))()
    end
})

local skyboxEnabled = false -- Default skybox state

-- Adding a toggle to the "Silent Aim" section
local toggleSkybox = silentaimCombat:Toggle({
    Name = "Custom Skybox",
    ...
    Callback = function(state)
        ...
        if not skyboxEnabled then
            -- Set skybox to default values if custom skybox is disabled
            -- You can replace these placeholders with the actual default skybox asset IDs
            game.Lighting.Ambient = Color3.fromRGB(127, 127, 127)
            local skybox = game.Lighting:FindFirstChildOfClass("Sky")
            if skybox then
                skybox.SkyboxBk = "rbxassetid://"
                skybox.SkyboxDn = "rbxassetid://"
                skybox.SkyboxFt = "rbxassetid://"
                skybox.SkyboxLf = "rbxassetid://"
                skybox.SkyboxRt = "rbxassetid://"
                skybox.SkyboxUp = "rbxassetid://"
            end
        end
    end
})

-- Adding a dropdown box with skybox options
local skyboxDropdown = silentaimCombat:Dropdown({
    Name = "Skybox",
    Options = {"DoomSpire", "Normal", "CatGirl", "Vibe", "Blue Aurora"},
    Default = "Normal",
    Callback = function(option)
        local skybox = game.Lighting:FindFirstChildOfClass("Sky")
        if skybox then
            if option == "DoomSpire" then
            ...
            elseif option == "Blue Aurora" then
...
local pageQOL = library:Page({ Name = "QOL" })
local cfgQOL = pageQOL:Section({ Name = "360 - THIS IS A WIP" })
local cfggQOL = pageQOL:InDevSection({ Name = "MISC - Macro doesnt work", Side = "Right" }) 


cfggQOL:Toggle({ Name = "Enabled", Pointer = "%enabled" }):Keybind({ Default = Enum.KeyCode.X, Pointer = "%keybind", Mode = "Always" })
cfggQOL:Slider({
    Name = "Speed",
    Min = 1,
    Max = 100,
...
local enable360 = false -- Default state

cfgQOL:Toggle({ Name = "Enabled", Pointer = "%enabled" }):Keybind({ Default = Enum.KeyCode.X, Pointer = "%keybind", Mode = "Always" })

-- Adding a slider called "Rotation Speed" to the "360" section
cfgQOL:Slider({
    Name = "Rotation Speed",
    Min = 1,
    Max = 10000,
...
settingsMenu:Textbox({Text = "Fate", PlaceHolder = "Custom Menu Name", Pointer = "custommenuname", Middle = false, ResetOnFocus = false, callback = function(name)
...
settingsMenu:Textbox({Text = "Fate | {fps} | {ping} | {time} | {date}", PlaceHolder = "Watermark Data",Pointer = "watermarkData", Middle = false, ResetOnFocus = false})
...
settingsConfig:Button({Name = "Save",confirmation = true, Callback = function()
    writefile("fate/configs/" .. shared.ConfigName, library:GetConfig())
    savedconfigsDropdown:Refresh(config:getFolder())
end})
settingsConfig:Button({Name = "Load",confirmation = true, Callback = function() 
    library:LoadConfig(readfile("fate/configs/" .. shared.SelectedCFG))
end})
settingsConfig:Button({Name = "Delete",confirmation = true, Callback = function() 
    if isfile("fate/configs/" .. shared.SelectedCFG) then
        delfile("fate/configs/" .. shared.SelectedCFG)
        savedconfigsDropdown:Refresh(config:getFolder())
        ...
        if library.pointers["%enabled"]:get() and library.pointers["%locktarget"]:get() then
            if input.KeyCode == library.pointers["%keybind"]:getkeybind() or input.UserInputType == library.pointers["%keybind"]:getkeybind() then
                ...
                elseif aimassistBool == false then
                    if fateConnections["aimassistConnection0172391"] then aimassistEvent:unbindConnection("aimassistConnection0172391") end
                end
            end
        end
    end
